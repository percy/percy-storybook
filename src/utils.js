...
/* istanbul ignore next: no instrumenting injected code */
export function evalStorybookStorySnapshots({ waitFor }, { docCapture = false, autodocCapture = false, relevantGraph = false } = {}) {
  ...
  return waitFor(async () => {
    ...
    if (relevantGraph) {
      ...
      stories.__relevantGraphDiagnostics = {
        source,
    ...
    let data = stories.map(story => serialize('snapshot', {
      ...
      id: story.id,
      ...(relevantGraph ? { importPath: story.importPath } : {}),
    ...
    return {
      ...
      data,
      ...(relevantGraph ? { diagnostics: stories.__relevantGraphDiagnostics } : {})
    };
  });
}
