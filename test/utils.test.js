...
describe('evalStorybookStorySnapshots importPath + diagnostics (IntelliStory)', () => {
  const waitFor = fn => Promise.resolve(fn());
  ...
  it('leaves importPath undefined when no source resolves one', async () => {
    ...
    const { data, diagnostics } = await utils.evalStorybookStorySnapshots({ waitFor }, { intelliStory: true });
  ...
  it('reports diagnostics for the preview-extract source', async () => {
    ...
    const { diagnostics } = await utils.evalStorybookStorySnapshots({ waitFor }, { intelliStory: true });
  ...
  it('does not extract importPath or diagnostics when IntelliStory is disabled', async () => {
    setPreview({
      extract: () => ({
        'button--primary': { id: 'button--primary', kind: 'Button', name: 'Primary', parameters: {} }
      }),
